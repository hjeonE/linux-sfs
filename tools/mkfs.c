...
int init_block_alloc_map() // BAM = Block Allocation Map
{
	char buffer[SFS_BLOCK_SIZE]; // 4KB temp allocate
	uint64_t *map = (uint64_t *) buffer; // 64bits 
	int i, block;
	...
	for (i = 1; i <= cfg.fs_bam_blocks; i++) {
		if (preallocated > BITS_PER_BLOCK) {
			// bits-per-block = 8 * sfs-block-size
